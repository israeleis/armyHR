# Diary Group-By Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add flexible group-by (unit · team · rank · status) to the `/diary` calendar screen's personnel list, matching the pattern used in SoldiersScreen and TransitionsScreen.

**Architecture:** One file change (`DiaryScreen.tsx`). Add `useGroupBy` state + `GroupBySection` inside the existing `FilterPane`. When group-by is active, replace the 6-item flat preview with a full collapsible list using `applyCollapse` over a flat item array. Headers and rows are flat siblings (not nested wrappers) — `applyCollapse` hides rows whose parent header is collapsed.

**Tech Stack:** React, Tailwind, existing `useGroupBy`/`applyCollapse`/`GroupBySection` from `@/features/filters`

## Global Constraints

- No new dependencies, no new files
- Hebrew labels: `יחידה · צוות · דרגה · סטטוס`
- Fallback labels: `ללא יחידה / ללא צוות / ללא דרגה / ללא סטטוס / אחר`
- `dir="rtl"` on all containers
- Tailwind only — no inline styles except `marginRight` for depth indentation and dynamic SVG colors
- Filter badge count = `filterCount + groupByKeys.length` (matching SoldiersScreen)
- No group-by active → keep existing flat preview (6 items + "see all" link)
- Group-by active → show ALL filtered soldiers for the date, grouped + collapsible, all sections start collapsed
...
| File | Change |
|------|--------|
| `src/features/diary/DiaryScreen.tsx` | Full replacement with group-by added |

---

## Task 1: Add group-by to DiaryScreen

**Files:**
- Modify: `src/features/diary/DiaryScreen.tsx`

**Interfaces:**
- Consumes: `useGroupBy<GroupByKey>()` → `{ groupByKeys, expanded, toggleExpanded, addGroupKey, removeGroupKey, moveGroupKey, reset: resetGroupBy }` from `@/features/filters/groupBy`
- Consumes: `applyCollapse(items, expanded): Array<Item & { isExpanded?: boolean }>` — items must ALL have `{ type: string; depth: number; path?: string }`. **Both header and row types need `depth: number`.**
- Consumes: `GroupBySection<K>({ options, groupByKeys, onAdd, onRemove, onMove })` from `@/features/filters/GroupBySection` — rendered as `children` inside `FilterPane`
- Consumes: `SoldierFields`, `StatusEntry` from `@/domain/types`

**Key pattern — flat list, not nested wrappers:**
`applyCollapse` returns a flat array. When a header is collapsed, its child items are filtered out of the array. In JSX, headers and rows are rendered as siblings. A header does NOT wrap its rows — it's a standalone `<button>` that calls `toggleExpanded(item.path)`. Rows appear directly after their parent header in the array.

- [ ] **Step 1: Replace `src/features/diary/DiaryScreen.tsx` with the full implementation**
...
// Both header and row items include depth for this reason.
...
Start dev server: `npm run dev`, open `http://localhost:5173/#/diary`

1. Filter icon in top-right — tap → FilterPane opens; "קיבוץ לפי" section is present at the bottom
2. Add "יחידה" → personnel section changes to grouped headers (unit names with counts), all collapsed; flat preview disappears
3. Filter badge shows `1`
4. Tap a unit header → `›` rotates 90°, soldiers for that unit appear below the header as indented rows
5. Add "דרגה" → multi-level: unit headers remain; tapping one reveals rank sub-headers inside
6. Remove all group-by → flat 6-soldier preview returns
7. Tap "נקה הכל" in FilterPane → both filter and group-by are cleared

- [ ] **Step 5: Commit and push**

```bash
git add src/features/diary/DiaryScreen.tsx
git commit -m "feat(diary): add group-by (unit/team/rank/status) to /diary screen"
git push origin feature/session-2026-06-26
```
